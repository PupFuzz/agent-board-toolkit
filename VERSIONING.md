...
Its **`artifacts` array is the must-move-together release set** — steps 3, 4 and 5 above, one entry each — and it is **asserted**, not merely printed: [`release-artifacts-gate.yml`](.github/workflows/release-artifacts-gate.yml) runs `bin/release-artifacts-check` on every PR, and a release PR (one whose `VERSION` **value** differs from its base's) fails if any declared member did not move, no longer exists at head, or does not carry the version being shipped. Before this the array was a `- [ ]` checklist in the generated body and nothing else, so a forgotten CHANGELOG heading or `CLAUDE.md` row shipped in silence. A **non**-release PR exits 0 without asserting anything, so the gate costs nothing on ordinary work; the post-merge `auto-tag-version.yml` stays fail-soft by design, because at that point the fix is a retag rather than a commit.
