...
7. Open the release PR `release/v<version>` → **`main`** with full release notes. **CRITICAL: the PR head must be the `release/v<version>` branch, NOT `dev` directly** — a `dev`-headed PR merged with auto-delete-head-branches enabled deletes `dev`.
    ...
    **Run the card-coverage check over the release range as part of preparing the body.** With `KANBAN_WRITEBACK_TOKEN` set, `promote-released-cards --dry-run` names every shipped ref that has no tracking card while fixing it is still cheap — otherwise the gap first surfaces as a red post-merge promote run needing board surgery. `release-pr-body` runs it for you and renders a `## Card coverage` section **only when it could actually measure** (board token + `.promote` config + the promote tool all present), so a generated body carrying **no** coverage section means the check **did not run** — not that it passed. The obligation lives here for the same reason as the merge-method rule in the next step: a body section whose whole content is "this was not checked here" is process narration, and it was struck by hand from a release PR that grew one — which does not hold, because the next release re-emits it.
