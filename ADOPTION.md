...
> **PM projects may vendor this directly (§8, amended 2026-06-19).** The standard originally scoped this toolkit to kanban-dev only. Because it's portable `bash` + `curl` + `jq`, a PM project (sola/aimla) whose host can run a shell **may vendor these tools** instead of reimplementing them — on the same terms as any consumer: **vendor + drift-check** against this repo, and route any shared-board need (e.g. the atomic DL-claim endpoint) through an **FR to this repo, never a silent fork**. Your *primary* board-ops sharing can still be your own runtime; this is an additional option, not a replacement. **Co-vendored lib:** if you vendor **by copy**, every *lib-sourcing* bin **requires `bin/_kb-board-lib.sh` copied beside it** (each `source`s it as a sibling). **That set is derived, never listed here** — a hand-kept list rots the moment a bin joins it, and this one had: `grep -lE '^[[:space:]]*source "\$KB_LIB"' bin/*` names it for the version you are actually vendoring. **The STANDALONE set is the complement, and is derived the same way rather than listed** — this sentence used to name three bins and read as the whole set, which it was not (it omitted `release-tag-check` among others, and would have gone stale again the moment a bin joined): the standalone tools are every public entry of `bin/` that the command above does **not** print, i.e. `for f in bin/*; do case "${f##*/}" in _*) continue;; esac; grep -qE '^[[:space:]]*source "\$KB_LIB"' "$f" || echo "${f##*/}"; done`. Run it for the version you are vendoring. ⚠ One standalone tool has a conditional sibling rather than a lib: `promote-released-cards --require-complete` calls `bin/card-completeness` (beside it, or on PATH) and refuses at rc 2 without it, so **co-vendor the two together if you pass that flag**; without the flag `promote-released-cards` needs nothing beside it, exactly as before. Omitting the lib makes each of those bins refuse with a message naming the lib and the fix — since v0.11.2 that self-naming message rather than the inscrutable `source: … No such file` it used to be — and it refuses on **every** invocation, `--help` included, because the lib is loaded before any argument is read; `board-card-start` reports the same and still exits 0, because it runs from a git hook that must never block a checkout. `agent-board-toolkit-drift-check` flags a lib-sourcing bin vendored without the lib.
...
**You may not need all of it if:** your project already shares board operations in another runtime (e.g. a Python framework/plugin) — this **bash** tooling doesn't replace that. But you can **vendor the whole toolkit** (§8 amendment above), cherry-pick the runtime-neutral pieces (`promote-released-cards`, `release-pr-body`, `release-artifacts-check`) — a GitHub-Actions repo can consume the promote step and the release-artifact gate as SHA-pinned composite actions instead of vendoring (INSTALL.md §6a / §6c), or just reuse the *patterns* (single-source + drift-check vendoring, the board-independent host-config split). Nothing here depends on a coordination framework.
...
Full step-by-step, troubleshooting, and the optional "consume a tool from a product repo's CI" paths (promote composite action §6a / vendor + drift-check §6b / release-artifact gate §6c): **[`docs/INSTALL.md`](docs/INSTALL.md)**.
...
Tools that do three things: **drive cards on the board** (the CRUD CLI, the session start/close reports, DL minting + card adoption, and the git hooks that move a card when work begins — the checkout move opt-in per repo), **cut and promote releases**, and **keep the install itself honest** (a currency check for a repo's vendored copy, and one for the `PATH` install).
