...
| `bin/release-artifacts-check` | assert every member of `.release-pr.json`'s `artifacts` array actually moved in a release PR — moved in the diff, still exists at head, and carries the version being shipped. The `artifacts` array's other consumer, `release-pr-body`, only **prints** it as a checklist; this one fails the PR. Classifies by version **value** (base == head ⇒ not a release PR ⇒ exit 0), so it is safe to run on every PR |
