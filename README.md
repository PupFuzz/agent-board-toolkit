...
| `bin/release-artifacts-check` | assert every member of `.release-pr.json`'s `artifacts` array actually moved in a release PR — present in the PR's own changes, still exists at head, and carries the version being shipped. The `artifacts` array's other consumer, `release-pr-body`, only **prints** it as a checklist; this one fails the PR. The range is `merge-base(base, head)..head`, never base's tip, so base-branch drift after the fork can neither fake a "moved" member nor hide a release; classification is by version **value** (fork point == head ⇒ not a release PR ⇒ exit 0), so it is safe to run on every PR |
