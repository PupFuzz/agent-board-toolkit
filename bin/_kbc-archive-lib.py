...
def make_github_state_resolver(kc, card: dict, cfg_repo: "str | None" = None):
    ...
    over it, and the card's derived source (payload.repo first, and only when it is a
    string containing `/`, else the URL keys — `docs/INSTALL.md` §4 states the whole
    order — via the framework's server-mirroring normalizer) is the final fallback."""
