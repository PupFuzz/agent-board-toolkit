...
- **UNMEASURED** — *indeterminate*: something that decides it could not be read, named on the line — a settings file this process cannot read or parse, a matcher or command it does not evaluate, a path that is not a checkout, or no `<repo-dir>` named at all. **"Could not check" never renders as LIVE.**
