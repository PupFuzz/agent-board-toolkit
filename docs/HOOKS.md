...
2. resolves the card (board id from the repo's `.release-pr.json` `promote.board_id` **or**, for a repo that doesn't cut releases, a repo-local `git config kanban.board-id`; API base from `.release-pr.json` or `~/.kanban-host.env`; in-progress stage from your `~/.kanban-<name>-board.env`), and verifies it is **on the repo's configured board** (so a stray number can't move an unrelated task; this is also the board-scope guard for the card-id fallback),
...
Requirements: the repo resolves a **board id** — either a `.release-pr.json` with `promote.board_id` (release repos) **or**, for a repo that doesn't cut releases, a repo-local `git config kanban.board-id <id>` (uncommitted; no `.release-pr.json` — and so no committed `api_base` — needed). You also have `~/.kanban-host.env`, a token file, and a `~/.kanban-<name>-board.env` whose `KB_BOARD_ID` matches the repo's board. The hook reads the token via `~/.kanban-host.env` (default `~/.kanban-dev-token`); a board whose token lives elsewhere needs `KBCARD_TOKEN_FILE` set in `~/.kanban-host.env`. Same config the rest of the toolkit uses (see [INSTALL.md](INSTALL.md)).
