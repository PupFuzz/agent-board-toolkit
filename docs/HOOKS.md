...
The remediation line is derived from the topology, not assumed: `install-board-hooks <repo>` is printed **only** where the directory that command would target is provably the one git dispatches from. Where a sibling checkout owns that directory (a linked worktree) the working path is named instead; where **no** invocation can service the checkout — a `.git` *file*, i.e. a linked worktree, a `--separate-git-dir` clone or a submodule, whose `<repo>/.git/hooks` target cannot be created — it says so and names the manual `ln -s` instead. Printing a command that exits 1 would be worse than printing none.

### What the reach test can and cannot tell you

It **reads** the hook — symlinks followed, `#` comments stripped — and follows one level of chaining, checking that the chained target is itself **executable** (git execs it; a non-executable target fails there exactly as a non-executable direct hook does). It never *executes* the hook, so its answer is a textual approximation with bounds in **both** directions. They are stated here and in the code because a check whose limits are undisclosed gets trusted past them:

- **Erring toward a false *finding* (safe).** A hook that only mentions `board-card-start` in a comment is reported as not reaching it; so is a hook that reaches the tool through a further indirection (a wrapper that is not itself a `hooks/<name>` path); and the `#` strip also blanks the tail of a line whose earlier text quotes a `#`.
...
The false-OK bounds are accepted deliberately: closing either requires **executing an arbitrary hook**, which a read-only session-close check must not do. Treat a `✓` as "a hook is wired and textually calls the tool", not as proof that a card will move. Both shapes are pinned by fixtures, so this disclosure and the behaviour cannot drift apart.
