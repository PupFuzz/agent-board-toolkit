...
It resolves the dispatch directory the way **git** does, not the way it is usually assumed — and the assumptions are where the silent no-ops live. It reuses `install-board-hooks`' own reader and resolver (one implementation, not a second copy), which means it inherits all three behaviours verified against git: `core.hooksPath` is read with `--path` so a leading **`~` is expanded** (git expands it; reading the raw value makes `~/hooks` look like a *relative* path and plants under the work tree); its **presence is taken from the exit status**, never from the value, because `--get` returns rc 0 with empty output for a set-but-empty value and rc 1 for unset — and those two mean opposite things; and the repo's git **common dir** is passed, so a linked worktree resolves to the main checkout's hooks dir. A check that read `.git/hooks` alone, or that read the value without its status, would reproduce the exact silent no-op the installer was fixed for — reporting a repo healthy on the strength of a hook git never runs.

The reach test **reads** the hook (with symlinks followed and `#` comments stripped) and follows one level of chaining; it never executes it. So a hook that only *mentions* `board-card-start` in a comment is correctly reported as not reaching it, while a mention inside a string literal still reads as a reach, and a hook reaching the tool through a further indirection reads as a finding. Those bounds are stated in the code and hold in the safe direction — a false *finding*, never a false all-clear.
