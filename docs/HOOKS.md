...
- **LIVE** — every fact the hook needs to act was read and holds.
...
**The dispatch hook** (`hooks/agent-dispatch-card-start`). LIVE means a `hooks.PreToolUse` entry whose matcher covers `Agent` runs a runnable file named `agent-dispatch-card-start` as a **bare path**, and `kbcard` is on `PATH`. It reads `${CLAUDE_CONFIG_DIR:-~/.claude}/settings.json`, the project's `.claude/settings.json` and `.claude/settings.local.json` (the project is `--project`, default the current directory — run it from where your sessions start), and the managed `managed-settings.json` where one exists, and lists each file with what it found. **Bounds, printed on every run, in both directions:** `claude --settings`, server-managed settings, MDM/OS policy (a macOS profile, the Windows HKLM/HKCU registry), a `policyHelper`, the SDK `managedSettings` option, `managed-settings.d` drop-ins, hooks in skill or subagent frontmatter, and plugin hooks are **not read** — so a registration made only there reads `NOT-REGISTERED`, and a `disableAllHooks` or `allowManagedHooksOnly` set only there can make a `LIVE` here wrong. A wrapper that runs the hook without naming `agent-dispatch-card-start` in its command or `args` is undetectable and reads `NOT-REGISTERED` too. A matcher is evaluated only as a literal (absent, `""`, `*`, `.*`, or a `|`/`,` list naming `Agent`); a regex is UNMEASURED. A command is evaluated only as **one shell-form word that is a bare path**, read with the quoting `sh -c` applies: a leading `~/` expands only unquoted, and `$HOME` / `$CLAUDE_PROJECT_DIR` only unquoted or inside double quotes — a quoted `~` or a single-quoted `$HOME` stays literal, so that command is a relative path and UNMEASURED. `bash <path>`, any other variable and other shell forms are UNMEASURED. A handler that names the hook — in its command or in any `args` element — while setting `args` (exec form: no shell), `if`, `once`, or a `shell` other than `bash` is UNMEASURED, naming the field. `disableAllHooks` / `allowManagedHooksOnly` set true makes the verdict UNMEASURED by name. **It never prints a hook command** — a command line can carry an env-prefixed secret — only the file, the matcher, the entry index and a resolved bare path. LIVE means the hook will *run*; it still acts only on a prompt carrying a `BOARD-CARD:` line (§ [Marker convention](#marker-convention-load-bearing--opt-in-per-dispatch)).
