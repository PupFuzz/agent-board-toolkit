...
**A checkout whose git dir is not `<root>/.git` is refused, and the refusal names the right target for its topology.** The installer services a checkout whose hooks directory lives inside its own `.git` directory. Three topologies break that assumption — and they need three *different* answers, so there is no single generic message:

| Topology | Where git dispatches hooks from | What the refusal tells you |
| --- | --- | --- |
| **Linked worktree** (`git worktree`) | the **main** checkout's `.git/hooks`, shared by the main checkout and every worktree | run `install-board-hooks <main-checkout>` — that wires this worktree too |
| **`--separate-git-dir`** | the separate git dir's `hooks/` | there is no second checkout to run it against; symlink the hooks in by hand |
| **Submodule** | `<superproject>/.git/modules/<name>/hooks` | running it against the superproject would wire a *different* repository; symlink by hand |

Only the worktree case has another checkout to redirect you to, which is why it is the only one that prints a command. Detection is by **git common dir** (`git rev-parse --git-common-dir` ≠ `<root>/.git`), never by `.git`-is-a-file: all three share that shape. Note that `--git-common-dir` ≠ `--git-dir` is **not** the discriminator — measured on git 2.43, those two differ **only** for a linked worktree; the other two topologies report them equal.
