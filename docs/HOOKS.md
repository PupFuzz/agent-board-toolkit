...
Treat a `✓` as "a hook is wired, is runnable, and textually calls the tool" — not as proof that a card will move. One bound is **not** covered and is disclosed rather than claimed: a shebang line longer than the kernel's buffer (`BINPRM_BUF_SIZE`, 256 bytes on Linux) is truncated, while the check reads the whole line. On the kernel this was verified against (6.8) the effect is that the *named interpreter* is silently ignored and the shell runs the file instead — the hook still fires, so it is a wrong-interpreter hazard rather than a dead hook; the effect may differ on other platforms.
