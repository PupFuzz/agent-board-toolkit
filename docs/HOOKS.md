...
`board-hooks-check <repo-dir>` reads it the way the hook does and reports **ARMED** beside **WIRED** — § [Is each hook live on this seat?](#is-each-hook-live-on-this-seat--board-hooks-check).
...
4. once that move has been **read back on the card** (card#10029 — the `PATCH`'s 2xx is the server accepting the request, not the board holding the stage), stamps the seat owner tag `owner:<project>/<seat>` in a separate write. When it is stamped, when it is not, and what is logged are [README § The seat owner tag](../README.md#the-seat-owner-tag--ownerprojectseat); a tag that is not stamped never undoes or fails the move.
...
Then check it took: `board-hooks-check` (run from the directory your sessions start in, or pass
`--project <dir>`) reports the hook **LIVE**, **NOT-LIVE** or **UNMEASURED** from the settings
files it actually read — § [Is each hook live on this seat?](#is-each-hook-live-on-this-seat--board-hooks-check).
...
## Is each hook live on this seat? — `board-hooks-check`

Every hook here is fail-soft, so a hook sitting in this repo and a hook that never runs on your seat look identical from every surface the toolkit writes. `board-hooks-check` is the one reader that says which, for **both** delivery mechanisms, and it is **read-only** — it installs, arms, registers and edits nothing:

```bash
board-hooks-check [--project <dir>] <repo-dir>…    # the checkouts you cut branches in
```

It prints one verdict line per hook, and the verdict is one of **three**, never two:

- **LIVE** — every condition this tool *reads* holds: the hook is registered or wired where the tool looks, points at an existing executable, nothing in what it read disables it, it is armed where arming applies, and the tools it calls are on `PATH`. **It is not a claim that the hook will run** — the runtime conditions the tool does not read are printed on every run as a fixed list (below).
- **NOT-LIVE** — *determined*: something it needs was read and is absent. The line names which: `NOT-REGISTERED`, not wired, not armed, or the tool the hook calls (`board-card-start`, `kbcard`) not on `PATH`.
- **UNMEASURED** — *indeterminate*: something that decides it could not be read, named on the line — a settings file this process cannot read or parse, a matcher, command or handler field it does not evaluate, a path that is not a checkout, or no `<repo-dir>` named at all. **"Could not check" never renders as LIVE.**

The exit status carries the same split, and it is `install-board-hooks`' own rather than a second vocabulary: **`0`** all LIVE · **`1`** at least one NOT-LIVE · **`4`** none NOT-LIVE but at least one UNMEASURED · **`2`** usage. `1` outranks `4` — both are non-zero, so neither reads as live, and the per-hook lines still carry each.

**The dispatch hook** (`hooks/agent-dispatch-card-start`). LIVE means: registered in a settings file this tool reads, as a `hooks.PreToolUse` entry whose matcher covers `Agent`, running an existing executable file named `agent-dispatch-card-start` as a **bare path**, with no `disableAllHooks` / `allowManagedHooksOnly` in those files, and `kbcard` on `PATH`. It reads `${CLAUDE_CONFIG_DIR:-~/.claude}/settings.json`, the project's `.claude/settings.json` and `.claude/settings.local.json` (the project is `--project`, default the current directory — run it from where your sessions start), the **main checkout's** `.claude/settings.local.json` when the project is a linked worktree, and the managed `managed-settings.json` plus its `managed-settings.d/*.json` drop-ins where they exist, and lists each file with what it found. A LIVE whose registration comes **only** from project files says on its line that it holds only if the folder is trusted (the workspace trust dialog, which the tool does not read). **Everything else is a bound, printed once on every run as a fixed list — that printed list is the authority, so run the tool to read it rather than trusting a copy:** launch flags that skip hooks or settings sources, workspace trust, managed-source precedence (a higher-ranked managed source can silence the managed file), which root `${CLAUDE_PROJECT_DIR}` names in a linked worktree (not verified, so from a worktree a command using it reads UNMEASURED even when it does not name the hook — a symlink or renamed copy need not — unless another registration reads LIVE), and the registration sources the tool never sees (a registration made only there reads `NOT-REGISTERED`). A matcher is evaluated only as a literal (absent, `""`, `*`, `.*`, or a `|`/`,` list naming `Agent`); a regex is UNMEASURED. A command is evaluated only as **one shell-form word that is a bare path**, read with the quoting `sh -c` applies: only space, tab and newline separate words (a trailing CR, VT or FF is part of the name, so UNMEASURED), a leading `~/` expands only unquoted, and `$HOME` / `$CLAUDE_PROJECT_DIR` only unquoted or inside double quotes — a quoted `~` or a single-quoted `$HOME` stays literal, so that command is a relative path and UNMEASURED. `bash <path>`, any other variable and other shell forms are UNMEASURED. A handler that names the hook — in its command or in any `args` element — while setting `args` (exec form: no shell), `if`, `once`, or a `shell` other than `bash` is UNMEASURED, naming the field. `disableAllHooks` / `allowManagedHooksOnly` set true makes the verdict UNMEASURED by name. **It never prints a hook command** — a command line can carry an env-prefixed secret — only the file, the matcher, the entry index and a resolved bare path. Even where it runs, the hook acts only on a prompt carrying a `BOARD-CARD:` line (§ [Marker convention](#marker-convention-load-bearing--opt-in-per-dispatch)).

**The git hooks** (`post-checkout`, `pre-push`), per `<repo-dir>`. **WIRED** and **ARMED** are separate facts on the line, because wired-but-unarmed is the state that reads as working:

```
── Git hooks: /path/some-repo ──
  dispatch dir: /path/some-repo/.git/hooks
  ✗ post-checkout: NOT-LIVE — WIRED=WIRED ARMED=no; not armed: kanban.automove-on-checkout is unset (unset means OFF) — arm it with: git -C /path/some-repo config kanban.automove-on-checkout true (docs/HOOKS.md § Arm it per repo)
  ✓ pre-push: LIVE — WIRED=WIRED (needs no arming)
```

WIRED is the [dispatch check](#is-the-hook-still-wired--the-dispatch-check)'s own reader — called, not copied — so it resolves the dispatch directory exactly as that check does and inherits its reach-test bounds (§ [What the reach test can and cannot tell you](#what-the-reach-test-can-and-cannot-tell-you)). ARMED is the hook's own read, byte for byte: `git config --bool --get kanban.automove-on-checkout` answering `true`; any other answer — unset, false, or a value git cannot read as a boolean, quoted in git's own words — is what the hook treats as OFF. Both hooks also need `board-card-start` on `PATH`. **The `PATH` it reads is its own**; a hook runs with the `PATH` git has at dispatch time — that, `git push --no-verify`, a per-command `git -c core.hooksPath=` and the reach test's textual limits are printed as the git hooks' own fixed bound once per run whenever a `<repo-dir>` is named. A `<repo-dir>` git will not open that holds no `.git` is **UNMEASURED** (not a checkout, like a path with nothing there, a bare repository or a directory inside a work tree); a checkout git **refuses** — dubious ownership, a config value it cannot expand — is **NOT-LIVE**, quoted in git's words, because git dispatches no hook there.

**Where it differs from the session-close dispatch check:** that leg is a host ritual over a fixed list of this host's checkouts and prints a verified wiring *remedy* per finding (which is why it runs `install-board-hooks --check`, with that dry run's probe write); this one takes the repos you name, answers *liveness* across both mechanisms, prints the arming command but no wiring remedy, and writes nothing.
...
- **Diagnostics (fail-soft but not silent).** The hook always `exit 0`s (it must never block a checkout), but when a branch carries a DL/card token and the move *didn't* happen for an infrastructure reason — no resolvable board id, an unloadable token/host, an untrusted `api_base`, unresolved stage ids, an unreachable board, a `card#N` that doesn't exist, a pinned card, or a `_kb-board-lib.sh` older than this hook (either a card-start invariant answering an rc outside its declared set — rc 127 — or the read-back's `kb_confirm_card` missing outright; on both routes **nothing is written, not even the `dl_number` stamp**, and the line names the missing function and the re-vendor — card#9756, card#10029) — it prints a one-line reason to stderr **and appends it to `~/.cache/agent-board-toolkit/board-card-start.log`** (`KB_BCS_LOG` overrides the path). Because the installed hook wrapper discards stderr, that log is the durable record: check it if a card you expected to move didn't — **after** confirming the repo is armed (`git config --bool --get kanban.automove-on-checkout`), because an unarmed repo never calls the mover and so leaves nothing in this log to find. Where a line names the api base, any **userinfo is masked to `***`** — an api_base may legitimately carry `user:password@`, and this log outlives the run on disk — so `https://***@board.example.com/api/v3` in the log means the base carried a credential, not that it is malformed. A branch with **no** token, a card already **past** the move stages, or a card-id number that lives on **another** board stays silent — those are genuine no-ops, not failures.
- **A write that answered 2xx is reported from a RE-READ of the card, not from the status (card#10029).** Both writes — the stage move and the `payload.dl_number` stamp — re-read the card afterwards and say what it holds: the success line quotes the value it read back (`… → In Progress (read back: workflow_stage_id=84)`), a re-read that MEASURES a different value logs **`NOT APPLIED`** quoting what the board holds, and a re-read that measures nothing (refused, never completed, or carrying no readable card) logs **`UNVERIFIED`** — nothing there says the write landed and nothing says it did not, so **re-read the card before acting either way**. A 2xx the board did not apply used to print the plain success line, which is how a seat could be told its card had moved while the board still held it in Backlog. The hook stays `exit 0` on every one of those outcomes; only what it SAYS changed. **The owner tag is stamped only after a move that was read back** — a move nobody could confirm gets no owner tag. ⛔ **The read-back's own lib function is checked for BEFORE the first write, by definedness rather than by calling it**, and that ordering is load-bearing rather than tidy: `kb_confirm_card` is called *after* the `PATCH`, so beside an older lib with no preflight the write would go out, the missing function would answer rc 127, and the run would report the write UNVERIFIED and exit 0 — a mis-vendored install degrading silently while the board changed. The preflight is what keeps *a lib older than this hook writes nothing* (card#9756) true of the read-back too.
