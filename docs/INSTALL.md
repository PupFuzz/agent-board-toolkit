...
> **Vendoring a *lib-sourcing* bin (not `promote-released-cards`)?** The interactive/hook bins — `kbcard`, `next-dl`, `board-snapshot`, `board-card-start`, `dl-a0-backfill-triaged`, `dl-a1-register-field` — `source` `bin/_kb-board-lib.sh` as a sibling, so you must copy **the lib too** into the same `bin/` (`cp ~/agent-board-toolkit/bin/_kb-board-lib.sh <repo>/bin/`). Without it the tool dies with a bare `source: …/_kb-board-lib.sh: No such file` at runtime; `agent-board-toolkit-drift-check` now flags a lib-sourcing bin vendored without the co-located lib. `promote-released-cards` / `release-pr-body` are standalone and need no lib.
