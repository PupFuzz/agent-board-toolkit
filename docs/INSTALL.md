...
**Also copy `LICENSE`** into `<repo>` alongside the vendored files — it carries the licensor's `Required Notice:` line, which the license requires you to pass on with any copy you distribute — or keep that line and the license URL in your own repo's NOTICE instead.
