...
**A vendored copy can say what it is, without a checkout beside it:** `<repo>/bin/<tool> --tool-version` prints the toolkit release the file was stamped at, from the file alone. Every `bin/` tool a composite action runs answers it (that set is derived by `tests/tool-version-stamp-selftest.sh` from the `*/action.yml` files, which is why no list is kept here), so this works on exactly the tools this section tells you to copy. A copy that refuses the flag as `unknown arg` is older than the flag, never a version. The drift-check above compares against a checkout; this is the question you can still ask when there is none.
