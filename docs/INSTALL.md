...
>
> **The board id has ONE source: this board env's `KB_BOARD_ID`** (card#10385). An exported `KB_BOARD_ID` is never a tier — it used to lose silently to this file's value, or, where the file set none, win while this file still supplied the stage and type ids, putting a card on one board in another board's stage. Every tool that resolves its board through the shared `kb_resolve_env` (`bin/_kb-board-lib.sh`) ignores it and prints `⚠ ignoring the inherited KB_BOARD_ID=…` when it differs from the id this file declares; `kbcard create-card` refuses (`rc 2`, no request) a board env that declares none, and names the board on stderr on every success (`card <id> created on board <n>`). Choose a board with `--board <name>` or `KBCARD_BOARD_ENV` (below).
>
