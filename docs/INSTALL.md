...
>
> **`unset_correlation_keys` — declaring a correlation key intentionally absent.** `promote.source`, `ref_token_regex` and `card_token_regex` are what a release correlates on, and every one of them fails **silently** when it is missing: `release-pr-body` emits a **0-id** `shipped-cards` manifest at **rc 0** under the line *"All shipped refs have a tracking card"* — a clean assertion about coverage nothing measured. That shipped on the consumer repos card#8423 enumerates before anyone noticed — that card owns the list, and no count is restated here. So `release-artifacts-check` (§6c) now refuses a config that declares a `promote` block and omits one of the three, and the one way through is to say so **by name**:
>
> ```json
