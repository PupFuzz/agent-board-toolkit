...
This is the local counterpart to the CI-side `KANBAN_EXPECTED_HOST` in §4/§5; one setting here lets card automation reach the board from every repo on the host — whether the checkout hook calls the mover at all is still each repo's own arming ([`HOOKS.md` § Arm it per repo](HOOKS.md#arm-it-per-repo--unset-means-off-card9845)). (CI jobs set it in their own env, not from this file.)
...
>
> **The board id has ONE source: this board env's `KB_BOARD_ID`** (card#10385). An exported `KB_BOARD_ID` is never a tier — it used to lose silently to this file's value, or, where the file set none, win while this file still supplied the stage and type ids, putting a card on one board in another board's stage. Every tool that resolves its board through the shared `kb_resolve_env` ignores it and prints `⚠ ignoring the inherited KB_BOARD_ID=…` when it differs from the id this file declares. A board env that declares none is refused (`rc 2`, no request) by every `kbcard` verb and by the other tools that load a board through the shared loader (the card#10385 entry in [`CHANGELOG.md`](CHANGELOG.md) names them), because an empty id does not fail on the wire: an external-ref lookup scoped to no board searches every board the token can see. (`next-dl` names the missing key and skips its board check instead; `board-snapshot` reports that board as unread.) `kbcard create-card` names the board on stderr on every success (`card <id> created on board <n>`). Choose a board with `--board <name>` or `KBCARD_BOARD_ENV` (below).
>
...
> ⛔ **MIGRATING ONTO THE POINTER MEANS DELETING THE COPY, and that step is the one an install forgets.** Dropping step (c)'s `KBCARD_TOKEN_FILE` line so the store's pointer resolves does **not** remove `~/.kanban-<name>-token`: the file stays on the box holding a live token, out of whatever rotates the store's, and nothing about the running install looks any different. Delete it in the same sitting — `rm ~/.kanban-<name>-token` — or, if you are keeping the per-board token, leave step (c) in place and let the pointer stay shadowed. **That shadowed-pointer install is supported, and the check below reports it as a `⚠` at rc 0 — reported, never a finding that gates**: a store whose kanban token belongs to the framework's own tooling while this seat's boards declare their own is exactly that shape, and the `⚠` says so in its own words. It becomes the `✗` arm only when the shadowed file holds the **same** token a board already resolves, which is the redundant copy this box is about.
>
> **`agent-board-toolkit-runtime-check` reports a seat that is carrying more than one (card#8376),** so this is a state you find out about rather than one you have to remember. **Its population is every DECLARED source** — this process's `KBCARD_TOKEN_FILE`, `~/.kanban-host.env`, every `~/.kanban-*-board.env`, the store's pointer — **plus every file matching the literal glob `~/.kanban-*-token`.** That second half is what sees the copy in the box above: dropping step (c) removes the only declaration naming it, so a declared-only check would report a seat carrying two tokens as `ok`. It compares what they hold **by digest, internally** — over the content its READERS see, so the file step (b)'s `printf '%s'` wrote with no trailing newline and the same secret an editor terminated are ONE credential here, exactly as they are to every tool:
>
> - **the same token in two files ⇒ ✗ and rc 1**, naming the file to delete and the one to keep (the store's pointer wins that choice, since the store already manages it) — and where the file to delete is one **no source this check walks declares**, `rm` is the whole remedy, because there is no config line to drop or repoint, **unless a board env reached only through `$KBCARD_BOARD_ENV` declares it**: that board is outside what the check consults (the second bound below), so it is named in the `✗` itself and is the one thing to check before deleting;
> - **a readable token file no board resolves, holding a different token ⇒ ⚠ and rc 0** — a second live credential, reported and not gated, because a store whose kanban token belongs to the framework's own tooling while every board here declares its own is a supported install; the `⚠` offers that reading as its third arm, so "nothing needs doing" is one of the answers it names rather than one you have to supply;
> - **several boards each with their own token, all in use ⇒ reported as `ok`, never as a finding.** Per-board isolation is the topology this section recommends, and the check does not report it as duplication. (Under `--quiet`, which is how `board-snapshot` runs it at SessionStart, an `ok` prints nothing at all — a `⚠` or a `✗` still does.)
>
...
>
> **Two bounds, stated because a check's edge is not guessable from its output.** A token file that **no source declares** and that the glob does **not** match — a `~/.kanban-dev-token.bak`, a copy under another directory — is outside the population and is not reported. And "no board resolves it" means every board env under **`~/.kanban-*-board.env`**: a board env parked elsewhere and reached only through `$KBCARD_BOARD_ENV` (see below) is outside what the check consults, and a token only that board resolves reads here as a second credential — or, if the seat also holds a copy of it, as an undeclared duplicate. **Both arms carry that bound in their own text**, the `✗` included, because the `✗` is the one that names a file to delete.
...
# edit: set promote.{board_id, released_stage_id, source}, ref_token_regex (e.g. "DL-[0-9]+"),
...
# promote.source: REQUIRED, no default. "<owner>/<repo>" if the board tracks SEVERAL repos,
# or "*" to DECLARE that it tracks exactly one — see the box below. promote-released-cards
# REFUSES to run until this key is present.
...
> **`ref_token_regex` and `card_token_regex` are TWO ID SPACES, not two spellings of one.** They are independent — set whichever your commit subjects actually use (set both if they use both) — but ⚠ **they are no longer optional for a repo that declares a `promote` block.** `release-artifacts-check` (§6c) refuses such a config unless both are declared, or the missing one is named in a `unset_correlation_keys` array; a repo with no `promote` block owes neither key. See the `unset_correlation_keys` box below for why the acknowledgement exists.
...
>
> **What each key buys.** `release-pr-body` reports card coverage on **stderr** — `release-pr-body: card coverage:` lines, never a body section — by dry-running `promote-released-cards` over the range's refs and naming any that have **no tracking card** — so a typo'd id, or a card belonging to a different board, is caught at release-prep rather than by a red post-merge promote run. (On a **repo-qualified** board that report carries **two** kinds, with opposite remedies — see the `promote.source` box in §4.) **It runs only when every one of its preconditions holds, and without any one of them the run prints no coverage line at all.** The set is stated once, in the `CARD COVERAGE PRECONDITIONS` block above `card_coverage_report()` in `bin/release-pr-body` — read it there rather than from a copy. ⚠ **The block ships only in a copy vendored from the toolkit release that carries card#10039, or later** — an older copy does not have it at all, and the precondition sentence it *does* carry is the falsified one that release corrects, so reading it as authority is worse than not reading it. That includes a copy redistributed by another project, whose lag behind this repo nothing in this tree can measure: run `grep -c 'CARD COVERAGE PRECONDITIONS' <your copy of release-pr-body>` before you trust one — **`0` means your copy predates this and you must upgrade it rather than read it.** A coverage line's presence IS the claim that coverage was measured, **or a named reason it could not be** — it never prints a placeholder saying it was not, because a report whose only content is "not checked here" says nothing about the release — one was struck by hand from a release PR body, and a hand-removal does not hold, since the next release re-emits it. ⚠ **The obligation therefore belongs in YOUR release doc:** state in your `VERSIONING.md` (or equivalent release flow) that `promote-released-cards --dry-run` is run over the release range **before merging**. A run with no coverage line means the check did not run — not that it passed. Each key also adds a machine-readable footer: `<!-- release-manifest:shipped-refs=DL-1,DL-2 -->` and `<!-- release-manifest:shipped-cards=5877,5874 -->` (bare ids — there is no token spelling for a consumer to parse).
>
> **`unset_correlation_keys` — declaring a correlation key intentionally absent.** `promote.source`, `ref_token_regex` and `card_token_regex` are what a release correlates on. A missing `promote.source` is loud (`promote-released-cards` refuses at rc 2), but the two token keys fail **silently**. With `card_token_regex` absent, `release-pr-body` emits **no** `shipped-cards` footer at **rc 0**, and when `ref_token_regex` still matches something no correlation-gap line fires and the card-coverage report can read *"All shipped refs have a tracking card"* — a clean line about coverage the range's card ids never received. That shipped on the consumer repos card#8423 enumerates before anyone noticed — that card owns the list, and no count is restated here. So `release-artifacts-check` (§6c) now refuses a config that declares a `promote` block and omits one of the three, and the one way through is to say so **by name**:
>
> ```json
> "unset_correlation_keys": ["card_token_regex"]
> ```
>
> The run then prints a `UNSET: card_token_regex` line naming the tool that goes quiet, and exits 0. Same two rules as `retired_artifacts`: a key that is **both** declared and listed is a config error (rc 2), so an acknowledgement cannot be planted over a live key; and a name that is not one of the three is a config error too — a misspelling acknowledges nothing, and would otherwise read as if it had. **A repo with no `promote` block is outside this entirely** and owes none of the three keys: the `promote` block is the claim that your releases move cards, and this check holds you to what that claim needs.
>
> A key that is present but **empty or whitespace-only** is refused as well (it is byte-identical to omitting it), and a token regex that `grep -E` cannot compile is refused with it — the readers pipe these values straight into `grep -oiE` behind a `|| true`, so an uncompilable regex does not fail, it matches nothing and yields the same empty manifest.
>
...
> **`promote.source` is REQUIRED and has no default — declare your board's ARITY.** `dl_number` and `pr_number` are **repo-scoped** counters: repo A's PR #15 and repo B's PR #15 are different pull requests carrying the same number. On a board that tracks one repo that is harmless; on a board that tracks several it is a collision, and `promote-released-cards` — which writes cards into a **terminal** stage — then promotes **another repo's** cards. Measured by a reporting peer on a board carrying three repos: one release's **15 shipped PRs matched 24 cards across all three**, including cards already released from a different repo's own version line.
>
> There is no safe default, for the same reason `KANBAN_EXPECTED_HOST` has none: a vendored tool cannot assume your topology, and the unsafe assumption here is a **write**. So the key is required and the tool refuses — naming this fix — until it is set.
>
> - **`"source": "<owner>/<repo>"`** — the repo this release ships (e.g. `"acme/widget"`). Every **ref**-matched card is qualified against it using the card's own by-ref source, derived exactly as the kanban server derives it: `payload.repo` (when it is a string containing `/`), else the first of `payload.pr_url`, `payload.issue_url`, `payload.html_url`, `external_link` that is a GitHub URL with a trailing `/pull/`, `/issues/`, `/commit/`, `/tree/` or `/blob/` segment. A card belonging to a different repo is **not promoted and is named**; so is a card with **no derivable source** — stamp it (`kbcard patch --pr-url https://github.com/<owner>/<repo>/pull/<N>`) and re-run, or promote it by hand. Neither is an error: both are pre-existing board conditions, reported on stderr and counted in the summary, and neither changes the exit code. Both also go to `$GITHUB_STEP_SUMMARY` under Actions, since a stderr line in a green job is invisible in a collapsed log. <!-- by-ref-source-rule: HOME — the operator-facing full statement. `tests/by-ref-source-claim-selftest.sh` holds the field order in the sentence above against the shipped `derive_source`, and refuses a copy of this rule anywhere in the tree that drops the `payload.repo` condition. -->
>
> <!-- by-ref-source-rule: POINTER --> ⚠ **Clearing or re-pointing a URL does NOT move the source of a card that carries a `payload.repo`.** The precedence above is not a tie-break that a later write can flip: `payload.repo`, when it is a string containing `/`, is the answer outright. So `kbcard patch --clear pr-url` on such a card cannot detach it from its repo, and `--pr-url` pointed at another repo cannot re-attribute it — on a card WITHOUT `payload.repo`, both do exactly that. Check which case you are in (`kbcard show --task <id>`) before acting on either.
>
>   **What release prep shows.** `release-pr-body`'s card-coverage report (stderr) gives the two cases on **separate lines, with opposite remedies** — read it as such:
>
>   - **⚠ Shipped refs with no tracking card:** no card of *this repo* carries the ref. That includes a ref whose only match was **another repo's** card (the promote log's `⊘` line names it). Remedy: **create or correct** a board card.
>   - **⚠ Shipped refs whose card carries no by-ref source:** the card **already exists** and carries the ref — it is missing only the repo-qualified source. Remedy: **stamp the card that is there**, `kbcard patch --pr-url …`. **Do not create a second card**, which is what this line exists to stop you doing.
> - **`"source": "*"`** — an explicit **declaration** that the board tracks exactly ONE repo, so a bare `dl_number`/`pr_number` is unambiguous on it. Behaviour is then byte-identical to before this key existed, summary line included. `*` is a declaration, **not** a wildcard match: use it only when the board really is single-repo.
> - **`--source "<owner>/<repo>"|"*"`** overrides the key for one run (a dry-run pointed at another repo). An explicitly empty `--source ""` is a config error, so an unexpanded shell variable can never fall through to whatever the committed config says.
>
> The value must be a bare `<owner>/<repo>`: a URL, a `.git` suffix, a glob like `owner/*`, a bare name, extra path segments or embedded whitespace are all refused by name — each is a spelling that would look configured while correlating to nothing. Case is folded (`ACME/Widget` and `acme/widget` are the same board arity), matching how the server stores a card's source. **`--cards` is exempt**, deliberately: a card *id* is not a repo-scoped counter, so an explicitly named id is promoted regardless of its source — and the run says so on stderr rather than doing it silently.
>
> ⚠ **Adopting this on an existing install is a one-line edit, and until you make it every promote run fails at rc 2.** Add the key to the `promote` block; single-repo boards want `"*"`. `release-pr-body`'s card-coverage report degrades gracefully in the meantime — it reports that the coverage check could not run rather than failing the body.
>
> ⚠ **And the value must be the repo it is declared in.** A resolved source other than `"*"` is compared against **`$GITHUB_REPOSITORY`** — case-folded on both sides — and a mismatch refuses at rc 2 **before any board read**. Nothing else could catch a typo'd or copy-pasted slug: it passes every shape check above and then correlates against no card at all, so every release promotes **0** and exits **0**. The leg does **not** fire when `$GITHUB_REPOSITORY` is unset (an operator's terminal, a vendored cron — there is no repo identity to check against there) and does not fire on `"*"` (a declaration, not a repo name). It applies to `--source` too: the flag overrides the *value*, and a run promoting another repo's cards is the failure, not the channel that asked for it. A **fork** running your release workflow is refused by the same leg, on its own `$GITHUB_REPOSITORY`.

> **The COMPLETENESS GATE — `--require-complete`, opt-in, and turning it ON is YOUR decision.** A card's `pr_number` holds ONE pull request, so a card whose work spans several reaches its shipped stage when whichever part merged FIRST merged — and this mover, which selects by that stage and writes into a TERMINAL one, then records the whole card as released while the rest is open or unreleased. With the flag, each matched card is put to `bin/card-completeness` (vendored beside the mover; `--completeness PATH` if you keep it elsewhere) and is REFUSED, by name, while a pull request whose HEAD BRANCH carries its token is still OPEN or was MERGED into your integration branch (`.dev_branch`, default `dev`) by a commit this release does not contain — and refused the same way when those pull requests cannot be READ, because a silence is not an answer. The run then exits **5** with every refusal named; the cards that ARE complete still move. Absent, nothing about the run changes.
>
> **What it costs you to switch on, before you do:** the oracle reads GitHub, so the job needs `$GITHUB_TOKEN`/`$GH_TOKEN` with **`pull-requests: read`** and `contents: read`; a release whose PRs the token cannot see refuses EVERY card rather than promoting one; a stale open PR nobody intends to land holds its card back until that PR is closed; and correlation is by CARD TOKEN on the head branch, so a repo whose branches name only a DL gets no protection from it. `bin/card-completeness`'s header owns the predicate and the full list of what it cannot see — read it there, and read a green run as "no unreleased PR NAMED this card", never as "this card is finished".
>
> ⚠ **`0 completeness-refused` at rc 0 is ALSO what a gate asked about the wrong population looks like, so the gate's `ON` line names that population: the resolved RELEASE HEAD and the INTEGRATION BRANCH, each with the channel it came from.** Both resolve from two channels and neither wrong resolution has any other symptom. With no `--head` the release head is whatever the checkout is at — run from an integration-branch checkout the window is empty and the gate has nothing to find. The integration branch is `.dev_branch` in the PR-editable `.release-pr.json`, which is **not** one of the correlation keys `release-artifacts-check` guards, so a PR pointing it at the release branch empties the window permanently. This is the same rule the `api_base` resolution above follows, applied to the gate.
...
>
> **Running it by hand** (the release-prep dry run `release-pr-body`'s card-coverage report also performs — and which needs all three variables, since a config carrying only the placeholder refuses before the first request):
>
> ```bash
...
- uses: <owner>/agent-board-toolkit/promote@<full-40-char-SHA>  # vX.Y.Z
  with:
    ...
    # require-complete: 'true'                # optional COMPLETENESS GATE — see the box in §4. Needs `permissions: pull-requests: read` (plus the `contents: read` the job already has) in the CALLING workflow, and REFUSES a card whose other PRs are still open or unreleased. Absent = off, byte-identical
    dry-run: ${{ github.event.inputs.dry_run }} # optional workflow_dispatch passthrough — ⚠ declare that dispatch input as `type: boolean`, NOT a free-text string (see the box below)
    # source: 'acme/widget'                    # optional OVERRIDE of the REQUIRED .promote.source (§4). Normally leave it out and set the key in .release-pr.json — the script refuses before any read if neither is present. Same literal-not-a-variable reasoning as shipped-stage-ids below, and stronger: a blank input here does NOT disable a guard, it falls back to the committed key
```

Pin by **full 40-char SHA with the `# vX.Y.Z` comment** (the comment is what dependabot parses). The consumer repo still needs its own `.release-pr.json` (§4) — **including `promote.source`, which is REQUIRED and has no default: without it every run of this action fails at rc 2, naming the key and the fix** — and unset-guards for the two repo variables if it wants friendlier errors than the script's own fail-closed ones.

> **The BOOLEAN inputs take `true` or `false`, lower-case, and REFUSE anything else at rc 2.** `require-complete` and `dry-run` are the two. An empty value — the input left out, or a `${{ }}` expression that resolves to nothing because the run was not a `workflow_dispatch` — still means `false`, so the passthrough above is safe on a push-triggered run. Everything else (`True`, `TRUE`, `yes`, `on`, `1`, and their negatives) is a refusal naming the input, **not** a value read as `false`: a composite action's inputs are strings, the test that consumed them was `[ "$X" = "true" ]`, and a near-miss spelling therefore produced a run byte-identical to one where the input was never passed — for `dry-run` that meant **real board writes for an operator who asked for a preview**, with nothing in the log to say so. ⚠ **If you wire `dry-run` to a `workflow_dispatch` input, declare that input `type: boolean`** — a `type: string` (or `type: choice` with `True` among its options) hands this action whatever the operator typed.
>
...
**Also copy `LICENSE`** — `cp ~/agent-board-toolkit/LICENSE <repo>/bin/LICENSE.agent-board-toolkit` (never over your own repo's LICENSE) — it carries the licensor's `Required Notice:` line, which the license requires you to pass on with any copy you distribute; or skip the copy and keep that line and the license URL in your own repo's NOTICE instead. Either satisfies the license's own requirement (the terms or their URL, plus the notice) — carrying `LICENSE` just does both in one file. `agent-board-toolkit-drift-check` skips any `bin/` file with no same-named counterpart in the toolkit, so this copy is invisible to it.

**A vendored copy can say what it is, without a checkout beside it:** `<repo>/bin/<tool> --tool-version` prints the toolkit release the file was stamped at, from the file alone. Which tools answer it is derived, not listed: `tests/tool-version-stamp-selftest.sh` holds every member to the flag, and its header (THE POPULATION IS DERIVED, THE REACH OF STEP 2) is the one statement of that set and of what it cannot see. It is not the same as what the sibling recipe below tells you to copy (for `adopt-to-dl` it lists `kbcard`, which does not answer the flag). For a tool the selftest's population holds, a copy that refuses the flag as `unknown arg` is older than the flag, never a version; for any other tool the refusal says nothing about its age. The drift-check above compares against a checkout; this is the question you can still ask when there is none.
...
**It also guards the CORRELATION keys, and this is the one leg that is not about `artifacts`.** A config declaring a `promote` block must declare `promote.source`, `ref_token_regex` and `card_token_regex`, or name the missing one in `unset_correlation_keys` (§4). Read at **head only** — there is no narrowing to catch, since deleting a key is what this reds on, and a fork-point read would red every consumer's next PR for a key their already-merged config never had. It runs **before** the empty-`artifacts` opt-out (the two are independent declarations: a repo that opted out of the artifact legs still promotes) and is **skipped under `--classify-only`**, whose caller (`release-tag-check`, post-merge on a push) cannot act on a config finding — the PR run is where it can be fixed, and every repo that runs the mode runs the full check on its PRs. It does **not** re-implement `promote.source`'s accept set: `promote-released-cards` owns that predicate, a malformed value is already a loud rc 2 there, and a third copy of a predicate that has already diverged once in service would buy a shift-left at the price of a false red the day the copies disagree.

> ⚠ **Price this before you bump the pin: the pin does not travel with the config.** On a `pull_request` event GitHub takes the **workflow** — hence the `uses:` SHA — from the **base-merged** ref, while the recipe above hands the action `pull_request.head.sha`. So the first time you bump this action's pin on your integration branch, **every branch you already have open that was cut before the correlation keys landed reds on a config it never touched.** It is not a config bug at that head and it must not be fixed as one: declaring the key again on a feature branch lands a duplicate declaration, and `unset_correlation_keys` there tombstones a key that is live on the base. **Merge the base branch** — and the author does not have to work that out: the refusal reads the key's state at the branch's fork point and at the base tip, and prints the one remedy those two facts select, together with the facts themselves. Branches that already carry the keys, and the release PR shape itself, are unaffected.

**It fails closed** (rc 2) on an unresolvable merge base, an unreadable version file, a config not committed at head, a member declared in both `artifacts` and `retired_artifacts`, a `promote`-declaring config missing (or mis-typing) an unacknowledged correlation key, or a version file that carries several values head introduced while its classifying match is unchanged — naming which end of the range it is and the path, rather than reading any of them as "not a release PR", which would be a silent non-run of the entire gate. A shallow checkout is the usual cause of the first two, hence `fetch-depth: 0`.
...
4. **`retired_artifacts` and `unset_correlation_keys` are head-editable narrowing surfaces by construction.** Each is declared, logged and one line; neither is impossible. Both are deliberately *declarations* rather than silences — the diff shows a named key changing and the CI log names what stopped being asserted — which is the whole difference from the state before them, where the narrowing left no trace anywhere. **Deleting the whole `.promote` block is a WIDER narrowing than either of them**, and it is the one that leaves no `UNSET:` line at all: the correlation leg is triggered by that block, so removing it removes the obligation rather than acknowledging it. It is self-limiting rather than guarded — `promote-released-cards` reads `.promote.board_id` / `.released_stage_id` and dies rc 2 without them (measured), so the post-merge promote job reds on the next release — but the red arrives one merge later than this gate would have, and it names the missing board keys, not the narrowing.
...
7. **A correlation key that is present, well-formed and WRONG.** This check asserts that the three keys are there, are non-empty strings, and (for the two regexes) compile — not that they match what your subjects carry, and not that `promote.source` names your repo. The first is reported by `release-pr-body`'s correlation-gap lines on stderr over each release range; the second is refused by `promote-released-cards` against `$GITHUB_REPOSITORY` (§4). Neither is answerable here: this check sees one config, not a range and not a runner.
