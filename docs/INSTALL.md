...
>
>   **What the release PR body shows.** `release-pr-body`'s `## Card coverage` section reports the two cases on **separate lines, with opposite remedies** — read it as such:
>
...
>
> ⚠ **Adopting this on an existing install is a one-line edit, and until you make it every promote run fails at rc 2.** Add the key to the `promote` block; single-repo boards want `"*"`. `release-pr-body`'s `## Card coverage` section degrades gracefully in the meantime — it reports that the coverage check could not run rather than failing the body.
>
