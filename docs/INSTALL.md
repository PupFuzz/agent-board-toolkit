...
>
> **Two bounds, stated because a check's edge is not guessable from its output.** A token file that **no source declares** and that the glob does **not** match — a `~/.kanban-dev-token.bak`, a copy under another directory — is outside the population and is not reported. And "no board resolves it" means every board env under **`~/.kanban-*-board.env`**: a board env parked elsewhere and reached only through `$KBCARD_BOARD_ENV` (see below) is outside what the check consults, and a token only that board resolves reads here as a second credential.
