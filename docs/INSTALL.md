...
>
> ⚠ **A red promote step is not always a failed one — `rc 3` means UNVERIFIED (card#9938).** Every move is reported from a re-read of the card, so the run separates three outcomes: `0` no card was measured in another stage and no write went unread (⚠ it does **not** mean every matched card is released — see the conditions below) · `1` **a card read back in another stage** (`move NOT APPLIED`, quoting what the board holds) · **`3`** a stage `PATCH` went out, answered success, and its outcome could not be read (refused, never completed, or a `2xx` carrying no readable stage). **Nothing in a `3` says those cards did not move, and nothing says they did** — the run names each one, and re-running the tool is safe (a card already at the target stage is skipped as already-released). The previous behaviour reported every `2xx` as a move and exited `0`, which is how a release once read as fully promoted with all 28 of its cards still in Shipped.
>
> **What the two non-zero codes are conditional on, stated rather than left to be assumed — a release promotes MANY cards, and a rule that holds only when the run promoted nothing is the one that was not worth having.** A card that **read back in another stage** is `rc 1` **whatever else the run did** — with 27 others moved, with an already-released one beside it, either way; the board was read and it disagrees with the write, so the release is not clean. **`rc 1` also outranks `rc 3`** on a run carrying both: `3` is the absence of a claim, `1` is a measurement, and the run's own `UNVERIFIED WRITE` line is still printed so the cards to re-read are still named. ⚠ **What is NOT unconditional is the REFUSED move** (`move failed (left in place)` / `move NOT CONFIRMED` — the server answered non-2xx, or the request never completed): that counts as `failed` and reaches `rc 1` **only on a run where no card moved and none was already released**, so a refusal beside a promoted card exits `0` with the refusal on stderr. That is the pre-card#9938 acceptance, unchanged deliberately — **if your gate must catch it, read the `<n> failed.` tail of the summary line rather than the exit code.**
