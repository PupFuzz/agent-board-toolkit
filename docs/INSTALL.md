...
**A vendored copy can say what it is, without a checkout beside it:** `<repo>/bin/<tool> --tool-version` prints the toolkit release the file was stamped at, from the file alone. Every `bin/` tool a composite action names answers it, and so does every sibling one of those launches in the one shape `tests/tool-version-stamp-selftest.sh` can see: a file resolving its own directory via `dirname` of `$0`/`BASH_SOURCE`/`__file__` that names the sibling literally as a `/<name>` path segment (that selftest derives the set from the `*/action.yml` files and the tools themselves, which is why no list is kept here). That set is not the same as what the sibling recipe below tells you to copy — the recipe also collects a sibling merely named in a message, which the set leaves out. Other `bin/` tools (`kbcard` and the lib-sourcing bins among them) do not answer it. **Weakest property, and a silent one:** the derivation cannot see a sibling whose name is held in a variable or assembled at run time, or a launcher that resolves its own directory another way (`${BASH_SOURCE[0]%/*}`, say) — such a sibling is not held to the flag and nothing reports it as unexamined. For a tool in the set, a copy that refuses the flag as `unknown arg` is older than the flag, never a version; for any other tool the refusal says nothing about its age. The drift-check above compares against a checkout; this is the question you can still ask when there is none.
