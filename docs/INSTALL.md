...
**A vendored copy can say what it is, without a checkout beside it:** `<repo>/bin/<tool> --tool-version` prints the toolkit release the file was stamped at, from the file alone. Every `bin/` tool a composite action names answers it, and so does every sibling one of those launches from its own directory (that set is derived by `tests/tool-version-stamp-selftest.sh` from the `*/action.yml` files and the tools themselves, which is why no list is kept here) — the release tools this section tells you to copy. Other `bin/` tools (`kbcard` and the lib-sourcing bins among them) do not answer it. For a tool in that set, a copy that refuses the flag as `unknown arg` is older than the flag, never a version; for any other tool the refusal says nothing about its age. The drift-check above compares against a checkout; this is the question you can still ask when there is none.
