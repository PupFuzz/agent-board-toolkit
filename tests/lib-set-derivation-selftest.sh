...
# The LIVE/FROZEN split helpers this leg uses — `_headings`, `_carve`, `_contains` — moved to
# `tests/_selftest-prelude.sh` when `tests/by-ref-source-claim-selftest.sh` became their second
# caller (canon #5). Their reasoning, including why the cut is by heading TEXT and what each
# caller still owes beside it, travelled with them and is not restated here.
