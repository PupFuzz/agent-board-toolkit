...
DISPOSITIONED=(
  ...
  "bin/card-completeness|LOSES|card#6911 instance — help path measured. Verdict path NOT separately driven; it writes its per-card COMPLETE/INCOMPLETE/UNMEASURED rows to STDOUT and exits 0/5/6 afterwards, so a truncating reader costs the rows while leaving the rc intact — and its one shipped consumer, \`promote-released-cards --require-complete\`, reads the ROWS, not the rc. That direction is fail-CLOSED there (a card with no row is skipped, never promoted), so a truncated read stalls promotions rather than minting them. It arrived with the gate (card#10176) carrying the shape it was adopted with, and is an instance to fix with its siblings rather than alone."
