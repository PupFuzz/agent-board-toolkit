...
# gained a `program_held == 0` term, so a withheld parent never raises a run's rc. Its rows are not
# in this file — the fixture that makes a parent is — so they live in
# `tests/promote-program-withhold-selftest.sh` § 6 (the rc-1 arm) and § 7 (the die), one each.
