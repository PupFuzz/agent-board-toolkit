...
EXTRACTORS=(
  ...
  "tests/next-dl-selftest.sh|2|TWO RESIDUALS, EACH OUT OF \`_fn_src\`'S REACH FOR ITS OWN REASON. (1) MISSING PRIMITIVE: TWO NAMES IN ONE READ. \`grep -E '^(max_int|max_dl)\\(\\) \\{'\` lifts BOTH of next-dl's one-line primitives at once and asserts it got exactly two lines. The ONE-LINE blocker is gone since card#8529 — \`_fn_src\` has a one-line mode — but it takes ONE name and answers ONE function, so it can express neither the alternation nor the exactly-two COUNT this leg asserts. \`reader-ref-canon-selftest.sh\` below is the same site's sibling, on \`max_int\` alone. (2) WRONG BODY SHAPE: A SUBSHELL FUNCTION (card#10230). \`ndl_fn_body\` lifts \`dl_sequence_call\`, which opens \`() (\` and closes on a column-zero \`)\` — \`_fn_src\` anchors \`<name>() {\` and closes on \`^}\`, so NEITHER half of its range applies and it would extract nothing. The anchor is written ONCE for the whole file and both static legs (the benign-exit scanner and the cause derivation) read that one function; the first draft spelled it in each of them and this leg caught the copy, which is the count that used to read 3."
  ...
  "tests/promote-refusal-detail-selftest.sh|1|A SCOPE FILTER, AND THE INVERSE OF AN EXTRACTION. One awk rule naming BOTH \`api\` and \`resp_detail\` in one alternation, so the scan that follows can SKIP their bodies and report every OTHER site in \`bin/promote-released-cards\` that touches the api-failure detail files (card#9301's class leg: resp_detail must be the only renderer). \`_fn_src\` is handed one name and answers with that function's TEXT; this leg needs the complement — the lines that are NOT in either of two functions — which it cannot express, and which subtracting two extractions from the file by text would re-implement worse. Nothing is read out of either function here."
  ...
  "tests/reader-next-action-census.sh|1|RESIDUAL, OUT OF \`_fn_src\`'S REACH BY SHAPE: IT NEEDS A LINE NUMBER, NOT SOURCE TEXT. One \`_ctl_line\` anchor, addressing the ONE-LINE \`die\` in \`bin/agent-board-toolkit-drift-check\` so LEG 3's control can classify that exact line and assert nothing below it is attributed to it — the \`; }\` function-extent bug this census was minted on. \`_fn_src\` answers with TEXT and has no line-number mode, and \`_ctl_line\` additionally asserts the anchor matches EXACTLY ONCE, which an extractor answering one function cannot express. Same shape, same reason, as \`mirror-pair-census.sh\` above; like the readback census it is not a selftest and sources no prelude."
  "tests/mirror-pair-parity-selftest.sh|6|SIX OCCURRENCES, THREE REASONS, NONE OF THEM AN \`_fn_src\`-SHAPED EXTRACTION. (1) A LOCATE, NOT AN EXTRACTION. \`^require_value[[:space:]]*\\(\\)\` over each shipped shell file answers WHICH FILES DEFINE the guard — that file's denominator, derived rather than listed so a fifth standalone copy is driven on the day it lands. The extraction itself is \`_fn_src\`, on the next line. \`_fn_src\` cannot express this: it is handed one file and asked for one function, and the question here is which files have one. (2) A NESTED, INDENTED FUNCTION, OUT OF \`_fn_src\`'S REACH BY SHAPE (card#10230). \`resp_excerpt\` is defined INSIDE \`dl_sequence_call\` in \`bin/next-dl\` at four-space indent and closes on an indented \`}\`; \`_fn_src\` anchors \`<name>() {\` at COLUMN ZERO and closes on \`^}\`, so neither end of its range applies and it would extract nothing. The site strips the indent afterwards, which \`_fn_src\` also has no mode for — the same residual \`locale-range-guard-selftest.sh\` above carries, for the same two reasons. (3) FOUR MUTATION ANCHORS ACROSS TWO LINES, and NOTHING is read out of a function by any of them. Each of the two control lines carries a \`sed\` substitution anchored at the already-extracted copy's own definition line, which RENAMES that copy (to \`rx_naive\`, resp. \`rd_naive\`) so the mutated control can be eval'd beside the shipped one — the planter shape \`url-userinfo-render-selftest.sh\` and \`token-duplication-selftest.sh\` are dispositioned for. ⚠ That substitution's literal is deliberately NOT quoted in this reason: quoting it would make THIS gate a member of its own population, and this line's own count would then move every time the sentence was edited. The OTHER anchor on each line is not an extraction at all: \`'[^']*'\` is the character class that cuts the scrub stage out, counted because \`[^\` wears the delimiter-plus-caret shape the scanner derives on. Spelling it some other way to duck the count would be writing for the gate rather than for the reader."
  "tests/board-card-start-selftest.sh|3|A MUTATION PLANTER AND ITS OWN CONTROL — NEITHER READS THE FUNCTION OUT. One \`sed\` RENAMES \`kb_confirm_card\`'s definition line in a copy of the lib, building the stale pairing the hook's preflight is driven against; the other two anchors \`grep -c\` that same definition line in the stripped copy and in an unedited sibling, asserting it is ABSENT from one and PRESENT in the other — the instrument control that says the \`sed\` matched something. \`_fn_src\` answers one function's TEXT and can express neither the rename nor the there-is-exactly-one-or-none property, and nothing here wants the text."
