...
#     is already excluded by the name rule — `__pycache__` is the one that shows up, no longer
#     from running the helpers (card#6871 stopped that) but still from a `py_compile bin/*.py`,
#     which is a CI step here — so a `-type f` filter would buy nothing and would silently drop
#     two real defects instead: a non-`_` directory dropped into bin/, and a tool that lost its
#     +x bit. Both stay in the set and read RED at whichever caller sees them.
