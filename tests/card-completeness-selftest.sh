#!/usr/bin/env bash
# card-completeness.selftest.sh — hermetic, network-free acceptance for the COMPLETENESS
# ORACLE (card#9869).
#
# WHY THIS FILE EXISTS. `card-completeness` is the single place that decides whether a card's
# work is finished, and the release movers refuse a TERMINAL board write on its answer. Both
# failure directions are silent and both are expensive:
#   - a false COMPLETE promotes a partially-implemented card into a stage nobody re-reads
#     (the defect itself — every sighting is listed once, in `card-completeness`'s header);
#   - a false INCOMPLETE stalls a finished card forever, which is card#9869's own closing
#     warning: "ANY FIX MUST NOT INVERT INTO A STALL".
# So every case below runs THE REAL SCRIPT end-to-end with `curl` stubbed on PATH — no
# network, no assertion on an exit code alone, and a control wherever a pass could be vacuous.
#
# THE OVER-COLLECTION CASES ARE THE POINT, not decoration. Derived on the framework repo's
# board 2026-09-20: matching an open PR against the card by BRANCH found 3 terminal-stage cards, by
# branch-or-title 4, and by ANY surface (body included) 10 — seven of the difference being a
# PR citing a prior card as background. § 3 and § 4 are that measurement, executable.
#
# RED-FIRST EVIDENCE — WHAT IS, AND IS NOT, CLAIMED. Every block carries a `RED when:` line
# naming a mutation of `card-completeness` that turns that block red. The line is a CLAIM, not a
# check: nothing in this file re-applies the mutation, so it is only as current as the last
# time someone applied it and watched the block fail. Each was last applied, and its block
# observed red, when card#9869's r1 review round changed this file; that run is not stored
# here, and nothing in this header asks you to trust it. Re-apply the mutation for any block
# you change, and watch it fail, before you trust that block's pass.
set -uo pipefail

HERE="$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")" && pwd)"
# TOOLKIT NOTE: bins live in bin/ here, not beside the test, so this is the one path that
# differs from the framework's copy of this file.
CC="$HERE/../bin/card-completeness"
[ -x "$CC" ] || { echo "selftest: $CC not found or not executable" >&2; exit 1; }
...
has_echo() { case "$2" in *"$1"*) echo true ;; *) echo false ;; esac; }
...
eq "  … and the PR is NAMED, with its branch"  "true"  "$(has_echo '#674 card-9606-bluetooth-disable-impl' "$(detail 9606)")"
eq "  … exit 5"                                "5"     "$rc"
...
eq "  … but WARNs, naming the PR"              "true"  "$(has_echo 'UNCORROBORATED' "$err")"
eq "  … and says which PR"                     "true"  "$(has_echo '#977' "$err")"
eq "  … and the WARN is on STDERR, not stdout" "false" "$(has_echo 'UNCORROBORATED' "$out")"
eq "  … exit 0"                                "0"     "$rc"
...
eq "  … and is not even warned about"          "false" "$(has_echo 'UNCORROBORATED' "$err")"
eq "  … exit 0"                                "0"     "$rc"
...
eq "  … naming the permission to grant"        "true"  "$(has_echo 'pull-requests: read' "$(detail 9606)")"
eq "  … exit 6 (more severe than 5)"           "6"     "$rc"
prs '[]'; page 1 status:404
run --cards 9606
eq "a 404 is UNMEASURED"                       "UNMEASURED" "$(verdict 9606)"
eq "  … naming the wrong-slug cause"           "true"  "$(has_echo 'wrong repo slug' "$(detail 9606)")"
prs '[]'; page 1 status:000
run --cards 9606
eq "a TRANSPORT failure is UNMEASURED"         "UNMEASURED" "$(verdict 9606)"
eq "  … naming it as transport"                "true"  "$(has_echo 'transport failure' "$(detail 9606)")"
eq "  … exit 6"                                "6"     "$rc"
...
# The accept-set is owned by BRIDGE-WRITEBACK.md and pinned across engines by the framework
# repo's promote-primitive-drift guard (framework-repo CI — it does not ship beside this file,
# so in YOUR repo the constant is bound by nothing but this case). What THIS case pins is that
# the oracle actually APPLIES it to a branch name — a correct constant read by nothing is a
# decoration, and that half holds wherever this suite runs.
...
echo "== 7b. TOOLKIT-LOCAL: the spellings THIS repo's own branches use =="
# ⛔ ADDED BY THE TOOLKIT (card#10176), and the reason is specific rather than belt-and-braces.
# This repo's `.release-pr.json` declares `card_token_regex: "card#[0-9]+"`, which is a TITLE
# grammar; the surface above is the HEAD BRANCH. Measured on this repo's refs, branches are
# spelled `card9939-…` (GLUED) and `fix/card-9756-…` (HYPHENATED, under a type prefix), and
# `card#[0-9]+` matches NEITHER — so a gate built on that key would match zero branches here and
# call every card COMPLETE. § 7 above pins the grammar in the abstract; this pins the two
# concrete shapes THIS repo produces, so a narrowing edit cannot pass by only satisfying the
# synthetic fixtures. Re-derive the shapes rather than trusting this comment:
#   git branch -a --format='%(refname:short)' | grep -i card
# RED when: `CARD_RE` drops the glued alternative (kills the first), or gains a left boundary
# that a `fix/` path prefix defeats (kills the second).
prs '[{"number":7001,"head":{"ref":"card9939-framework-mirror-check"},"title":""},
      {"number":7002,"head":{"ref":"fix/card-9756-card-start-unexpected-rc"},"title":""}]'
run --cards "9939,9756"
eq "a GLUED branch (card9939-…) correlates"        "INCOMPLETE" "$(verdict 9939)"
eq "  … naming the real branch"                    "true" "$(has_echo '#7001 card9939-framework-mirror-check' "$(detail 9939)")"
eq "a HYPHENATED branch under fix/ correlates"     "INCOMPLETE" "$(verdict 9756)"
eq "  … naming the real branch"                    "true" "$(has_echo '#7002 fix/card-9756-card-start-unexpected-rc' "$(detail 9756)")"
# CONTROL — the arm above is only meaningful if this same fixture can also answer COMPLETE.
run --cards "9939,9756,4242"
eq "  (control) an uncited card is COMPLETE"       "COMPLETE" "$(verdict 4242)"
...
eq "  … page 2 was actually fetched"           "true"  "$(has_echo 'page=2' "$(cat "$CALL_LOG")")"
...
eq "  … and says the read was TRUNCATED"       "true"  "$(has_echo 'TRUNCATED' "$(detail 9606)")"
eq "  … exit 6"                                "6"     "$rc"
...
# an argument to an external command — the property bin/_api_seam.sh exists to hold for its
# callers, which this vendored standalone cannot source. THE CONTROL is the second assertion:
...
prs '[]'
run --cards 9606
eq "the token is in NO argv"                   "false" "$(has_echo "$GITHUB_TOKEN" "$(cat "$ARGV_LOG")")"
eq "  … control: it IS on stdin"               "true"  "$(has_echo "$GITHUB_TOKEN" "$(cat "$TMP_STDIN_LOG")")"
...
eq "tokenless: no Authorization on stdin"      "false" "$(has_echo 'Authorization' "$(cat "$TMP_STDIN_LOG")")"
...
eq "  … naming the refusal"                    "true"  "$(has_echo 'refusing to send a credential' "$err")"
run --cards
...
eq "  … naming it"                             "true"  "$(has_echo 'no --release-head' "$(cat "$TMP/err")")"
eq "  … and no verdict lines"                  ""      "$out"
rc=0; out="$("$CC" --release-head relhead0 --cards 9606 2>"$TMP/err")" || rc=$?
eq "no --integration → rc 2"                   "2"     "$rc"
eq "  … naming it"                             "true"  "$(has_echo 'no --integration' "$(cat "$TMP/err")")"
...
eq "  … and refuses to guess"                  "true"  "$(has_echo 'Refusing to guess the population' "$(cat "$TMP/err")")"

echo "== 11. --help prints the WHOLE header and nothing but the header =="
# Same contract as the two movers: a LINE-COUNT EQUALITY against the file's own comment block,
...
eq "--help states the PREDICATE"               "true"  "$(has_echo 'THE COMPLETENESS PREDICATE' "$helpout")"
eq "--help leaks no script body"               "false" "$(has_echo 'set -euo pipefail' "$helpout")"
...
eq "  … naming it, and WHY it blocks"          "true"  "$(has_echo '#959 card-9451-waypoint-refs (merged to dev, not in the release head)' "$(detail 9451)")"
eq "  … exit 5"                                "5"     "$rc"
eq "  … the compare is <release-head>...<integration>" "true" "$(has_echo '/compare/relhead0...dev?' "$(cat "$CALL_LOG")")"
eq "  … EVERY window commit was looked up"     "true"  "$(has_echo '/commits/facade0/pulls' "$(cat "$CALL_LOG")")"
eq "  … a TITLE-only merged citation WARNs"    "true"  "$(has_echo '#960' "$err")"
eq "  … and does not block"                    "false" "$(has_echo '#960' "$(detail 9451)")"
...
run --cards 9451
eq "a BLANK title does not shift the row"      "true"  "$(has_echo '#959 card-9451-waypoint-refs (merged to dev, not in the release head)' "$(detail 9451)")"
...
eq "  … saying the window read is TRUNCATED"   "true"  "$(has_echo 'TRUNCATED' "$(detail 9451)")"
eq "  … exit 6"                                "6"     "$rc"
prs '[]'
...
eq "  … naming the unpushed/wrong-branch cause" "true" "$(has_echo 'unknown to GitHub' "$(detail 9451)")"
prs '[]'
window 1 c0ffee0; cpr c0ffee0 status:403
run --cards 9451
eq "a failed per-commit lookup is UNMEASURED"  "UNMEASURED" "$(verdict 9451)"
eq "  … naming that commit"                    "true"  "$(has_echo 'c0ffee0' "$(detail 9451)")"
...
eq "  … page 2 was actually fetched"           "true"  "$(has_echo '/compare/relhead0...dev?per_page=100&page=2' "$(cat "$CALL_LOG")")"
...
eq "  … naming the cap"                        "true"  "$(has_echo "${cap}-commit cap" "$(detail 9451)")"
eq "  … and looked up NO commit"               "false" "$(has_echo '/commits/' "$(cat "$CALL_LOG")")"

if [ "$fails" -gt 0 ]; then
  echo "card-completeness.selftest: $fails check(s) FAILED" >&2
  exit 1
fi
echo "card-completeness-selftest: all checks passed"
