...
DISPOSITIONED=(
  ...
  "bin/board-card-start:root|SAME OUTCOME — on the mover both take the same fail-soft bcs_skip; this hook writes nothing and makes no board claim on that path. On --lint both reach the card-id floor leg's 'NOT CHECKED … maps to no board' line, which says the floor was not consulted rather than that the branch is clean. Residual: the TEXT names one of the two causes."
  "bin/board-card-start:board|SAME OUTCOME — an unresolvable board id from either source takes the same bcs_skip on the mover (nothing is written), and on --lint the same 'NOT CHECKED … maps to no board' line, whose text names absent, unreadable and jq-not-on-PATH together rather than claiming one."
  "bin/board-card-start:board_envf|SAME OUTCOME — on the mover no per-board env is the normal case and the token falls back to the host default, which is also what an unreadable env leaves in place. On --lint an empty value reaches the floor leg's 'NOT CHECKED … no board env' line, never a clean verdict. Residual: kb_board_env_for skips an unreadable env file, so that line says 'no env has KB_BOARD_ID' where one exists but cannot be read."
