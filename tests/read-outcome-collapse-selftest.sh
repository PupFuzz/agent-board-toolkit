...
DISPOSITIONED=(
  ...
  "bin/kbcard:cmd_comments:comments|SAME OUTCOME for every body no comment list can be read out of — unparseable, no .data or a .data that is not one card object (an API or gateway error envelope, card#10489), a comments value that is not a list of objects (false included: absent-or-null is decided on the near side of the default), or a second JSON text — empty (or a second line) REFUSES, saying nothing was read and that this is NOT an empty comment list, while a card carrying no comments decodes to [], which is non-empty."
  ...
  "bin/kbcard:cmd_show:data|SAME OUTCOME — empty REFUSES show at rc 1, saying nothing was read: a body jq cannot parse, one with no .data or a .data that is not a card object (an API or gateway error envelope, card#10489), and a second JSON text (a second line) all land there. A card that was read is an object, which is never empty."
