...
DISPOSITIONED=(
  ...
  "bin/kbcard:cmd_comments:comments|SAME OUTCOME for every body no comment list can be read out of — a body that is not exactly one JSON text whose .data is an object (unparseable, a second text or trailing bytes even beside a card text, no .data as in an API or gateway error envelope, card#10489; _kbc_one_card answers empty, so no list is read), or a card whose comments is present and not null but not a list of objects with a string-or-absent content (false included: absent-or-null is decided on the near side of the default) — empty REFUSES, saying nothing was read and that this is NOT an empty comment list, while a card carrying no comments decodes to [], which is non-empty."
  ...
  "bin/kbcard:cmd_show:data|SAME OUTCOME — empty REFUSES show at rc 1, saying nothing was read: _kbc_one_card answers empty for every body that is not exactly one JSON text whose .data is an object — unparseable, a second text or trailing bytes even beside a card text, no .data (an API or gateway error envelope, card#10489), or a .data that is not an object. A card that was read is an object, which is never empty."
