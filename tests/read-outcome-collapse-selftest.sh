...
DISPOSITIONED=(
  ...
  "bin/board-card-start:board|SAME OUTCOME — an unresolvable board id from either source takes the same bcs_skip on the mover (nothing is written), and on --lint the same 'NOT CHECKED … maps to no board' line, whose text names absent, unreadable, not-a-plain-integer and jq-not-on-PATH together rather than claiming one."
