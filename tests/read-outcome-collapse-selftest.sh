...
DISPOSITIONED=(
  ...
  "bin/kbcard:cmd_comments():envelope|OPEN (card#10489) — '.data.comments // []' answers [] for a 2xx JSON body with no .data (an API or gateway error envelope), so 'kbcard comments' prints no comments at rc 0 for a card it never read. Refusing it moves a READ verb from rc 0 to rc 1, an acceptance change not granted. The 'false' half of the same default is docs/CONSOLIDATION-PLAN.md's 'shape test downstream of //' class."
  ...
  "bin/kbcard:cmd_show:data|SAME OUTCOME — empty (a body jq cannot parse) REFUSES show at rc 1, saying nothing was read. Residual, recorded in card#10489's tracking and not fixed here: a 2xx JSON body with no .data prints 'null', which is not empty, so this test never sees it."
