...
DISPOSITIONED=(
  ...
  "bin/board-card-start:root|SAME OUTCOME — on the mover both take the same fail-soft bcs_skip: no card moves and no board claim is made. On --lint an empty root leaves the board empty, which no decided verdict record can match (resolved and absent are only ever written with a board), so the board verdict leg speaks — NOT RECORDED, STALE or NOT CHECKED — and the card-id floor leg's 'NOT CHECKED … maps to no board' rides on that line; never a clean verdict. Residual: the TEXT names one of the two causes."
  "bin/board-card-start:board|SAME OUTCOME — an unresolvable board id from either source takes the same bcs_skip on the mover (no card moves; the verdict record says 'not checked'). On --lint no decided verdict record can match an empty board — resolved and absent are only ever written with one — so the board verdict leg speaks (NOT RECORDED, STALE or NOT CHECKED), and the floor leg's 'NOT CHECKED … maps to no board' rides on that line, its text naming absent, unreadable, not-a-plain-integer and jq-not-on-PATH together rather than claiming one."
  "bin/board-card-start:board_envf|SAME OUTCOME — on the mover no per-board env is the normal case and the token falls back to the host default, which is also what an unreadable env leaves in place. On --lint it is read only once the board verdict leg has left the branch undecided (and said so), and an empty value reaches the floor leg's 'NOT CHECKED … no board env' finding on that same line, never a clean verdict. Residual: kb_board_env_for skips an unreadable env file, so that line says 'no env has KB_BOARD_ID' where one exists but cannot be read."
  ...
  "bin/board-card-start:cbd|DISPOSED — reached only after cur, read by the same jq from the same already-accepted body, yielded a stage; empty is a parsed body with no board_id, which moves nothing and is recorded as 'not checked', never as a confirmed card on another board."
